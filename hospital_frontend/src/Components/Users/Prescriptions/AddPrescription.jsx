...
export default function AddPrescription() {
  ...
  async function onSubmitHandler(event) {
    ...
    const formData = {
      patient: name,
      providedBy: doctor.current.id,
      medicines: JSON.stringify(prescribedMedicines),
      notes: notes,
    };

    try {
      const response = await fetch(`${Conn}/prescriptions/add`, {
        method: "post",
        headers: {
      ...
      if (response) {
        ...
        if (response.ok) {
          toast.loading(
            "Successfully added prescription, redirecting to priscriptions page",
            { duration: 1800 }
          );
          handleNavigation();
        } else {
  ...
  let handleNavigation = debounce(() => {
    navigate("/users/prescriptions");
  }, 2000);
        ...
        <Form onSubmit={onSubmitHandler}>
          ...
          >
            ...
            {/* <TextField
              ...
              helperText={
                error.doctorNameError.state && error.doctorNameError.message
              }
              size="medium"
              disabled
            />
            <TextField
              name="uniqueId"
              id="uniqueId"
              label="Doctor's unique id"
              type="text"
              value={doctor.current.userId}
              helperText={
                error.doctorNameError.state && error.doctorNameError.message
              }
              size="medium"
